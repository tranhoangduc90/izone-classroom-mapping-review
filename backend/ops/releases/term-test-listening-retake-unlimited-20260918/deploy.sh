...
# Nhận chuỗi Compose của API hiện hành và image đã build trên VPS.
# Kiểm đúng image nền, source và cấu hình trước khi chỉ dựng lại API.
# Khi lỗi, dùng --rollback để trở về image trước mà không sửa dữ liệu thi.
service='mapping-review-api'
base_image='izone-term-test-backend:20260916.4-progress-log-checkpoint-retry'
new_image='izone-term-test-backend:20260918.1-listening-retake-unlimited'
release_dir='/opt/izone-listening-retake-unlimited-20260918'
...
# Lấy giá trị cấu hình từ container đang chạy, không in bí mật ra log.
...
if [[ "$mode" == '--rollback' ]]; then
  ...
  expected_version='1.8.8-k56-checkpoint-retry'
else
  docker image inspect "$new_image" >/dev/null
  export LISTENING_RETAKE_UNLIMITED_BUILD_SHA="$(docker image inspect "$new_image" --format '{{.Id}}' | sed 's/^sha256://')"
