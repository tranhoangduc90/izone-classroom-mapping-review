...
base_image='izone-term-test-backend:20260920.1-teacher-session'
new_image='izone-term-test-backend:20260920.2-dashboard-access'
...
export TEACHER_SESSION_BUILD_SHA="$(docker image inspect "$base_image" --format '{{.Id}}' | sed 's/^sha256://')"
...
if [[ "$mode" == '--rollback' ]]; then
  ...
  export DASHBOARD_ACCESS_BUILD_SHA="$(docker image inspect "$new_image" --format '{{.Id}}' | sed 's/^sha256://')"
