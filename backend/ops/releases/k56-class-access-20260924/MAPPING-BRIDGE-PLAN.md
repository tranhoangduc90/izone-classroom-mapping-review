...
| B3 | Hai migration cổng quyền + trạng thái đủ điều kiện, rồi image backend → kho K56 vẫn chỉ mở IC2264 | Chạy migration `202609240001` trước `202609240002`, sau đó image exact-source; readback production đúng ba hàng quyền IC2264, lớp khác đóng và học viên hợp lệ vẫn vào được. Không nhập roster trước B3. |
...
| B5 — lõi local đã chuẩn bị, chưa chạy live | Roster B4 + phạm vi ERP mới nhất → bật cặp lớp–đề | Đúng 29 × 3 cặp `enabled`, lớp ngoài phạm vi đóng; smoke HTTP đủ ba đề/K56 và K67 không đổi. |
| B6 — giao dịch local đã qua test, chưa chạy live | Lượt đồng bộ tiếp theo → đối soát trạng thái đủ điều kiện và quan sát | Giữ roster/UUID/bài cũ, chỉ đổi `is_eligible` cho học viên rời hoặc quay lại và tắt quyền lớp rời phạm vi. Lưu số lượt ERP để chặn lượt cũ; cần duyệt riêng khi giảm phạm vi. Chỉ lên lịch sau khi đã thử rollback và readback live. |

Kiểm B3 sâu hơn trên source K56 production bằng `stage_live_gate.py`: lớp vá hẹp sáu file có 10/10 ca SQL nhưng chỉ đạt 6/7 test backend tập trung. Source live chưa tự suy quyền mọi lớp từ `role=admin` và chưa trả `accessMode` ở danh sách lớp kiểu K56; **không hạ bộ test để phát hành bản vá hẹp**. Bản ứng viên hợp nhất dùng source live làm nền, áp cổng quyền và hòa giải các module từ branch đã đối chiếu; lệnh `--unified-candidate --full-suite` đạt 196/196 test backend, 10/10 ca SQL, không skip và không ghi production. Dấu SHA-256 của cây `src` ứng viên đã thử là `759a22ea355dff58d6497cb1343d12ddc5125c4d9b9afa6037a6d5fa592306ca`; lần build phải tái tạo đúng dấu này hoặc kiểm thử lại từ đầu. Audit hai profile thấy 30 module JS ở K56 live, 37 ở K67 live; so với branch, 10 module khớp cả hai, 1 khớp riêng K56, 18 khớp riêng K67 và 8 khác cả hai, cần review theo hợp đồng trước khi đóng B3. Kết quả thử trong thư mục tạm **không phải image đã build/deploy hoặc xác nhận giao diện**. B3 vẫn `not_ready` cho đến khi có bản phát hành exact-source, backup/restore, kiểm thử live và phê duyệt production. Tùy chọn `--compat-k56-live` chỉ dùng để chẩn đoán khác biệt API cũ, không phải cách bỏ qua blocker.

`plan_revision=k56-mapping-bridge-v2`. Bản v2 thêm trạng thái đủ điều kiện độc lập với roster lịch sử. Nếu chọn kết nối trực tiếp hai mạng/DB thay cho snapshot một chiều, B1–B6 trở thành stale và phải rà lại bảo mật, quyền, rollback trước build.
...
Lõi B4 trên branch gồm `prepare_import_payload` (chỉ tạo hàng mới trong RAM) và `roster_import.mjs` (đối chiếu lại toàn bộ đích dưới khóa bảng, nhập trong một giao dịch và đọc lại trước commit). Hàng cũ không bị cập nhật; lớp mới chỉ có ID/mã ERP, mặc định `pending_review`, không gán Classroom. Kiểm live phát hiện `row_count=510` của lượt nguồn đếm toàn bộ snapshot, còn 447 hàng `on_going` mới đủ điều kiện; bộ đọc nay đối chiếu cả tổng 510 lẫn 447 riêng, không loại sai 63 hàng ngoài phạm vi. Lõi B5 gồm `prepare_access_payload` và `class_access_enable.mjs`: chỉ bật quyền khi ba roster đủ điều kiện khớp hoàn toàn theo đề–lớp–học viên–UUID và baseline quyền chưa đổi; hàng lịch sử vẫn có thể hiển thị kết quả nhưng không được tính vào quyền dự thi. Chạy lại không ghi đè pilot. Full suite backend và bản stage ghép source live đều đạt 196/196, 0 skip; 10/10 ca SQL cổng quyền vẫn đạt. Đây **chưa** là lệnh chạy production: còn cần gói thực thi kiểm image có cổng quyền, snapshot nguồn mới nhất, backup/restore drill, HTTP smoke và phê duyệt riêng. Không gọi B4/B5 `done` chỉ vì lõi local qua test.

Lệnh nền hiện tại: 24 test Python của tuyến xem trước/lập hàng nhập/quyền/đối soát và cổng quyền đạt; bản stage hợp nhất và `npm test` trong `backend/` đạt 201/201; `npm run check` đạt. `python tools/writing-benchmark/audit-k56-cohort-readiness.py` trước triển khai trả `not_ready`, 1/29 mapping, 1/29 roster theo đề, bảng quyền chưa có. Sau B4/B5, cùng lệnh audit phải trả `ready` với số lớp/lượt học viên của snapshot mới nhất và ba đề đủ quyền. Trước mỗi release còn phải chạy full suite trên commit cuối, quality-gate checker của lát backend, HTTP smoke K56/K67 và đối chiếu số lượng/UUID ở database đích.

B6 đã có hợp đồng dữ liệu local: migration `202609240002` thêm `is_eligible` mặc định `true` và checkpoint lượt ERP; SQL vào bài K56 yêu cầu cờ này, còn truy vấn kết quả giảng viên vẫn giữ roster lịch sử. Bản xem trước `--audit-eligibility` chỉ in số hàng cần bật/tắt và cặp lớp–đề cần đổi; khi có học viên rời lớp hoặc lớp rời phạm vi, `manualReviewRequired=true`. `prepare_reconcile_payload` chỉ lập dữ liệu trong RAM sau B4/B5 và yêu cầu xác nhận đúng lượt ERP nếu có giảm phạm vi; `eligibility_reconcile.mjs` khóa bảng, kiểm baseline, cập nhật cờ/quyền/checkpoint trong một giao dịch và đọc lại, lỗi thì rollback. Regression PostgreSQL trong RAM kiểm nghỉ/quay lại, lớp rời phạm vi, lượt cũ, baseline đổi và lỗi giữa giao dịch; UUID/bài cũ nguyên vẹn, K67 không cần cột mới. **Chưa thử rollback live, chưa có nhịp tự động hoặc quan sát production nên B6 chưa hoàn tất.** Nếu lượt tiếp theo có drift, dừng mở quyền mới và báo cáo, không đồng bộ nửa chừng.
