...
Bản stage tạm từ 30 module source K56 live và lớp vá sáu file đã chạy thêm bốn file test backend: 6/7 ca đạt, một ca không đạt về metadata quyền admin của lớp. Source live K56 hiện không tự suy quyền toàn lớp từ `role=admin` và query legacy chưa trả `accessMode`; **không phát hành lớp vá hẹp sáu file**. Bản stage hợp nhất thêm các module branch đã hòa giải trên source live: `stage_live_gate.py --unified-candidate --full-suite` đạt 187/187 test backend và 10/10 ca SQL trong RAM, 0 skip, 0 ghi production. Dấu SHA-256 cây `src` ứng viên này là `7f13defc697bbc48e070b1f4dd00f390d72bc844b07acefb1e036d98bb303f90`; phải trùng ở image sắp build hoặc kiểm lại. Công cụ dọn toàn bộ thư mục stage sau kiểm. `audit_unified_live_drift.py` đối chiếu hash hai profile: 10 module khớp cả K56/K67 live, 1 khớp chỉ K56, 18 khớp chỉ K67 và 8 khác cả hai; phải rà kỹ các module khác biệt trước image release. Chưa có image phát hành hoặc bằng chứng người dùng nhìn thấy quyền admin trên production.
...
Lõi nhập roster một chiều đã có trên branch: bản lập hàng mới dùng khóa ERP/đề trong RAM, không tự duyệt Classroom; bản giao dịch khóa bảng và so lại đích trước khi chèn, đọc lại sau khi chèn, lỗi thì rollback. Kiểm live xác nhận `row_count=510` đếm mọi hàng snapshot, còn 447 hàng `on_going` được chọn; bộ đọc kiểm cả hai số tương ứng. 14 fixture Python và 9 test PostgreSQL đạt; chưa có gói chạy production vì image cổng quyền và backup/readback chưa đạt cổng B3.
