...
Full backend tại `cc28fd7` đạt 170/170 test, skipped 0; `npm run check` đạt. Phần kiểm xác thực Google giả lập bổ sung sau commit này cần chạy lại full suite trên revision mới. Cổng U1 vẫn **in progress**: thiếu parity endpoint trên bản thử với hai schema đầy đủ, kiểm các nhánh lỗi và readback; tuyệt đối không dùng bảng verdict này để khởi động deployment production.
