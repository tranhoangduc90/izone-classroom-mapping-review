...
7. `POST /api/term-tests/result` với `attemptToken` để mở đúng kết quả và đọc lại hai bài Writing của lượt làm đó. Sau khi Writing được nộp hoặc tự thu, PostgreSQL tạo riêng hai lượt chấm Task 1/Task 2, lưu trạng thái thử lại và chỉ trả điểm khi mỗi Task đã đủ bốn tiêu chí. Điểm Writing tổng được tính theo trọng số Task 1 × 1, Task 2 × 2 rồi làm tròn theo bước 0,5. Học viên nhấn điểm từng Task để xem đề, bài làm và nhận xét chi tiết; không có phần phân tích câu.
...
`attemptToken` và bản dự phòng cục bộ được giữ trong bộ nhớ trình duyệt của máy đang thi để đóng tab rồi mở lại vẫn tiếp tục được. Nháp Listening, Reading, hai bài Writing và tiến độ chấm đồng thời được lưu trong PostgreSQL; deadline luôn tính bằng giờ máy chủ. Lark Base chỉ là nơi tương thích với bộ chấm Writing 56/67 hiện có, không phải nguồn trạng thái retry. Dashboard công khai và luồng xuất Lark/Portal không đọc nguyên văn bài viết. Định nghĩa đáp án nằm trong PostgreSQL production; đề/audio computer-based nằm trong thư mục riêng chỉ mount vào container, không nằm trên GitHub Pages. API áp dụng CORS, giới hạn tần suất, UUID không tuần tự và không ghi payload học viên vào log lỗi.
